...
fn validate_event(
    ...
    if event.schema != "carrack.vfs.catalog-watch.v1"
        ...
        || carrack_sdk_core::validate_catalog_checkpoint_etag(&event.etag).is_err()
    {
        return Err(Error::CatalogWatch(
...
fn valid_nonzero_hex<const N: usize>(value: &str) -> bool {
    carrack_sdk_core::decode_lower_hex::<N>(value)
...
mod tests {
    ...
    fn event(revision_id: u64) -> CatalogWatchEvent {
        CatalogWatchEvent {
            ...
            etag: carrack_sdk_core::catalog_checkpoint_etag(&"22".repeat(32)).expect("valid etag"),
        }
    }
