//! Shared command surface for the native Skydriver binaries.

use clap::{Parser, Subcommand, ValueEnum, error::ErrorKind};
use serde::Serialize;
use serde_json::Value;
use skydriver_client::{
...
pub enum Error {
    ...
    #[error("invalid Skydriver CLI arguments: {0}")]
    Arguments(String),
    /// Native client failure.
    #[error(transparent)]
    Client(#[from] skydriver_client::Error),
    /// Structured output serialization failure.
    #[error("encode Skydriver CLI output: {0}")]
    ...
    #[error("invalid Skydriver CLI input: {0}")]
    Input(String),
    /// A committed receipt did not match the effective state read back from the server.
    #[error("Skydriver management verification failed: {0}")]
    Verification(String),
}
...
struct FilesystemArguments {
    /// Skydriver control-plane base URL.
    #[arg(long, env = "SKYDRIVER_CONTROL_URL", global = true)]
...
/// Runs one native Skydriver command with a stable binary identity.
///
/// # Errors
///
...
fn write_version(output: Output, surface: Surface) -> Result<(), Error> {
    write_json(
        ...
        &VersionOutput {
            ...
            protocol_epoch: skydriver_client::PROTOCOL_EPOCH,
        },
    )
}
...
fn require_control_url(control_url: Option<String>) -> Result<String, Error> {
    control_url.ok_or(Error::MissingEnvironment(
        "SKYDRIVER_CONTROL_URL or --control-url",
    ))
}

fn vfs_client(control_url: Option<String>) -> Result<VfsClient, Error> {
    let endpoint = require_control_url(control_url)?;
    let encoded = std::env::var("SKYDRIVER_VFS_TOKEN")
        .map_err(|_| Error::MissingEnvironment("SKYDRIVER_VFS_TOKEN"))?;
    Ok(VfsClient::new(&endpoint, VfsToken::parse(&encoded)?)?)
}

fn admin_client(control_url: Option<String>) -> Result<AdminClient, Error> {
    let endpoint = require_control_url(control_url)?;
    let account = std::env::var("SKYDRIVER_OPERATOR_ACCOUNT")
        .map_err(|_| Error::MissingEnvironment("SKYDRIVER_OPERATOR_ACCOUNT"))?;
    let encoded = std::env::var("SKYDRIVER_OPERATOR_CREDENTIAL")
        .map_err(|_| Error::MissingEnvironment("SKYDRIVER_OPERATOR_CREDENTIAL"))?;
    Ok(AdminClient::new(
        &endpoint,
    ...
    std::env::var_os("HOME")
        ...
        .map(|home| {
            ...
            if legacy.exists() {
                ...
                home.join(".local/state/skydriver")
            }
        })
...
fn error_disposition(error: &Error) -> ErrorDisposition {
    let (code, exit_status) = match error {
        ...
        Error::Client(skydriver_client::Error::InvalidEndpoint(_)) => ("invalid_control_plane", 4),
        Error::Client(skydriver_client::Error::UpgradeRequired(_)) => ("sdk_upgrade_required", 5),
        Error::Client(
            skydriver_client::Error::InvalidCompatibility(_)
            | skydriver_client::Error::InvalidResponse(_),
        ) => ("invalid_control_plane_response", 6),
        Error::Client(skydriver_client::Error::Failure { kind, .. }) => match kind {
            skydriver_client::FailureKind::MissingAuthority => ("permission_denied", 7),
            skydriver_client::FailureKind::UnsupportedSuite => ("unsupported_suite", 15),
            skydriver_client::FailureKind::CorruptCiphertext => ("corrupt_ciphertext", 16),
            skydriver_client::FailureKind::CorruptPlaintext => ("corrupt_plaintext", 17),
            skydriver_client::FailureKind::ProviderUnavailable => ("provider_unavailable", 18),
            skydriver_client::FailureKind::PermanentLoss => ("permanent_loss", 19),
        },
        Error::Client(skydriver_client::Error::Rejected {
            status: 401 | 403, ..
        }) => ("permission_denied", 7),
        Error::Client(skydriver_client::Error::Rejected { status: 404, .. }) => ("not_found", 8),
        Error::Client(skydriver_client::Error::Rejected { status: 409, .. }) => {
            ("revision_conflict", 9)
        }
        Error::Client(skydriver_client::Error::Rejected { .. }) => ("request_rejected", 10),
        Error::Client(
            skydriver_client::Error::Transport(_) | skydriver_client::Error::CatalogWatch(_),
...
mod tests {
    ...
    fn exposes_stable_machine_readable_exit_statuses() {
        ...
        assert_error_disposition(
            &Error::Client(skydriver_client::Error::InvalidEndpoint(
                "invalid".to_owned(),
            )),
        ...
        assert_error_disposition(
            &Error::Client(skydriver_client::Error::UpgradeRequired(Box::new(
                skydriver_client::UpgradeRequired {
                    ...
                    upgrade_command: "upgrade skydriver".to_owned(),
                },
            ))),
        ...
        assert_error_disposition(
            &Error::Client(skydriver_client::Error::InvalidResponse(
                "invalid".to_owned(),
            )),
        ...
        assert_error_disposition(
            &Error::MissingEnvironment("SKYDRIVER_VFS_TOKEN"),
            "missing_environment",
            14,
        );
        for (kind, code, status) in [
            (
                skydriver_client::FailureKind::UnsupportedSuite,
                "unsupported_suite",
                15,
            ),
            (
                skydriver_client::FailureKind::CorruptCiphertext,
                "corrupt_ciphertext",
                16,
            ),
            (
                skydriver_client::FailureKind::CorruptPlaintext,
                "corrupt_plaintext",
                17,
            ),
            (
                skydriver_client::FailureKind::ProviderUnavailable,
                "provider_unavailable",
                18,
            ),
            (
                skydriver_client::FailureKind::PermanentLoss,
                "permanent_loss",
                19,
            ),
        ] {
            assert_error_disposition(
                &Error::Client(skydriver_client::Error::Failure {
                    kind,
