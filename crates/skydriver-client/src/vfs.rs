...
impl VfsClient {
    ...
    pub(crate) async fn catalog_checkpoint(
        ...
        let request_headers = condition
            ...
            .map(|(value, revision)| {
                vec![
                    ("Skydriver-Catalog-Accept-Delta", "v1"),
                    ("Skydriver-Catalog-Base-Revision", revision),
                    ("Skydriver-Catalog-Base-Root", value.root_data_root.as_str()),
                    (
                        "Skydriver-Catalog-Base-SHA256",
        ...
        let expected_sha256 = required_header(&response.headers, "skydriver-catalog-sha256")?;
        let expected_etag = required_header(&response.headers, "etag")?;
        validate_catalog_checkpoint_etag(expected_etag)
            .map_err(|error| Error::InvalidResponse(error.to_string()))?;
        let expected_revision = required_header(&response.headers, "skydriver-catalog-revision")?
            .parse::<u64>()
            .map_err(|_| {
                Error::InvalidResponse("catalog checkpoint revision header is invalid".to_owned())
            })?;
        let expected_root = required_header(&response.headers, "skydriver-catalog-root")?;
        ...
        if content_type == "application/vnd.carrack.catalog-delta+json" {
            ...
            let expected_delta_sha256 =
                required_header(&response.headers, "skydriver-catalog-delta-sha256")?;
    ...
    pub async fn list_all(&self, directory_id: &str) -> Result<DirectoryPage, Error> {
        ...
        let mut accumulator = skydriver_sdk_core::DirectoryMerkleAccumulator::new();
...
fn validate_acl_policy(policy: &AclPolicy, directory_id: &str) -> Result<(), Error> {
    ...
    for grant in &policy.grants {
        ...
        if !skydriver_sdk_core::VFS_ACTIONS.contains(&grant.action.as_str())
            || grant
                .source_role
                .as_deref()
...
fn validate_placement_policy(policy: &PlacementPolicy, directory_id: &str) -> Result<(), Error> {
    ...
    for placement in &policy.placements {
        if !valid_bounded_string(&placement.driver_id, MAXIMUM_DRIVER_ID_BYTES)
            || skydriver_driver_contract::DriverKind::parse(&placement.driver_kind).is_none()
...
fn canonical_actions(actions: Vec<String>) -> Result<Vec<String>, Error> {
    skydriver_sdk_core::canonicalize_vfs_actions(actions)
...
fn validate_nonzero_hex_input<const N: usize>(value: &str, context: &str) -> Result<(), Error> {
    let decoded = skydriver_sdk_core::decode_lower_hex::<N>(value)
...
fn authenticate_directory_entries(
    accumulator: &mut skydriver_sdk_core::DirectoryMerkleAccumulator,
        ...
        let delivery = server
            .mock_async(|when, then| {
                ...
                then.status(200)
                    ...
                    .header(
                        "Skydriver-Catalog-SHA256",
                        "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                    )
                    .header("Skydriver-Catalog-Revision", "1")
                    .header("Skydriver-Catalog-Root", root)
                    .body(body);
            })
            .await;
        assert!(matches!(
