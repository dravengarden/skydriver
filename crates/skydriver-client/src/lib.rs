//! Native Skydriver control-plane client primitives.

pub use skydriver_sdk_core::{
...
/// Incompatible Skydriver wire-protocol generation implemented by this client.
...
pub enum Error {
    ...
    #[error("Skydriver control-plane request failed: {0}")]
    Transport(#[from] reqwest::Error),
    /// The optional catalog-watch acceleration channel failed.
    #[error("Skydriver catalog watch failed: {0}")]
    CatalogWatch(String),
}
...
fn validate_compatibility(response: &ProtocolCompatibility) -> Result<(), Error> {
        ...
        || response.upgrade_command.is_empty()
    {
        return Err(Error::UpgradeRequired(Box::new(UpgradeRequired {
            ...
            message: "Skydriver protocol or SDK version is incompatible".to_owned(),
