...
fn valid_driver_configuration(driver: &DriverRow) -> bool {
    match driver.kind.as_str() {
        LOCAL_FILESYSTEM_KIND => serde_json::from_str::<LocalFilesystemConfig>(&driver.config_json)
            .is_ok_and(|config| valid_local_root(&config.root) && driver.credential_present == 0),
        _ => false,
    }
}
...
#[cfg(test)]
mod tests {
    use super::{DriverRow, valid_driver_configuration, valid_local_root, valid_string};
    ...
    fn validates_identifiers_and_local_roots() {
        ...
        assert!(!valid_string(" local-main", 256));
        assert!(valid_local_root("/srv/carrack"));
        assert!(!valid_local_root("/srv/../secret"));
        assert!(!valid_local_root("relative"));
    }
